...
//************************************************************************
/*
	$Id: common_images.php,v 1.23 2005/10/05 03:55:59 ralfoide Exp $
...
//*********************************************************************
function rig_get_pict_link($album, $file)
//*********************************************************************
{
	global $abs_album_path;
	global $georef_shorttext;
	global $georef_longtext;
	global $georef_link;

	# In case we get called more than once, cache the result
	if ($georef_text)
	{
	    return array('shorttext' => $georef_shorttext, 'longtext' => $georef_longtext, 'link' => $georef_link);
	}

	$abs_link_file = $abs_album_path . rig_prep_sep($album) . rig_prep_sep(".rig_mapping");

	# Rig mapping file is: Pic_Name|Short Link Text|Long Link Text|<a href=/perso/gps/gmap/20080702_Lost_Coast_Trail.php?lat=40.0449581&lon=-124.0786916&name=photo&desc=446_LCT_Day2&label=photos&url=/perso/hiking/Pix/20080702_Lost_Coast_Trail/446_LCT_Day2.html&thumbnail=/perso/hiking/Pix/rig-cache/20080702_Lost_Coast_Trail/prev640_446_LCT_Day2.jpg target=20080702_Lost_Coast_Trail>

	if (is_readable($abs_link_file))
	{
	    $handle = fopen($abs_link_file, "rt");
		...
		if (preg_match('/([^|]*)\|([^|]*)\\|([^|]*)\|(.*)/', $line, $regs))
		{
		    ...
		    if ($regs[1] == $file)
		    {
			$georef_shorttext = $regs[2];
			$georef_longtext = $regs[3];
			$georef_link = $regs[4];
			return array('shorttext' => $georef_shorttext, 'longtext' => $georef_longtext, 'link' => $georef_link);
		    }
		    else
		    {
			// echo "No Match $regs[1]<BR>";
		    }
		}
		else
		{
		     rig_html_error("Error: can't process line from $abs_link_file: $line");
		}
	    }
	}
